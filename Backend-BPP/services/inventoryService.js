...
exports.getAvailableProductsWithMeta = async ({ productName, userLocation, radiusKm }) => {
  ...
  const query = `
    ...
    WHERE 
      ...
      AND (
        6371 * acos(
          cos(radians($3)) * cos(radians(SPLIT_PART(l.gps, ',', 1)::double precision)) * 
          cos(radians(SPLIT_PART(l.gps, ',', 2)::double precision) - radians($4)) +
          sin(radians($3)) * sin(radians(SPLIT_PART(l.gps, ',', 1)::double precision))
        )
      ) <= $5;
  `;
