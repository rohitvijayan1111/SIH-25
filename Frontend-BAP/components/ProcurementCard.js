...
const ProcurementCard = ({ item }) => {
  ...
  return (
    <View style={tw`bg-white p-4 rounded-lg border-gray-300 border-2 m-2`}>
      <View
        style={tw`-mx-4 flex-row justify-between items-center border-b-2 border-gray-300 pb-2 pl-2 pr-2`}
      >
        ...
        >
          <Text
            ...
            onPress={() => navigation.navigate('ProcurementDetails')}
          >
            More Details
          </Text>
        </TouchableOpacity>
      </View>
      {/* <View style={tw`w-full h-px bg-gray-300 -mx-4`} /> */}
      ...
      <TouchableOpacity
        ...
        style={tw`p-2 rounded-lg mt-1 p-2`}
        onPress={() => navigation.navigate('ProcurementDetails')}
        className='bg-[#2B9846]'
      >
        <Text
