...
export default function ProductDetailsScreen() {
  ...
  const API_URL = `http://${process.env.IP_ADDRESS}:5000/bap/select`;
  const SEARCH_URL = `http://${process.env.IP_ADDRESS}:5000/bap/search`;
  ...
  return (
    <ScrollView className="bg-white p-4">
      ...
      <Text className="text-xl font-semibold mt-1">{item.descriptor.name}</Text>
