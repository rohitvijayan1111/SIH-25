...
class BatchModel {
  ...
  factory BatchModel.fromJson(Map<String, dynamic> json) {
    return BatchModel(
      id: json['batch_id']?.toString() ?? '',
      batchCode: json['batch_code']?.toString() ?? '',
      quantityKg: _parseDouble(json['available_qty']),
      farmerName: json['farmer_name']?.toString() ?? 'Unknown',
  ...
  static DateTime _parseDate(dynamic value) {
    if (value == null) return DateTime.now().add(Duration(days: 30));
    if (value is String) {
      try {
...
class Batch {
  ...
  factory Batch.fromJson(Map<String, dynamic> json) {
    return Batch(
      ...
      farmerLocation: json['farm_location']?.toString(),
      organicCertified:
          json['organic_certified'] == true ||
          json['organic_certified']?.toString().toLowerCase() == 'true',
...
class BatchHistoryItem {
  ...
  factory BatchHistoryItem.fromJson(Map<String, dynamic> json) {
    return BatchHistoryItem(
      batchId: json['id']?.toString() ?? json['batch_id']?.toString() ?? '',
      batchCode: json['batch_code']?.toString() ?? '',
      productName: json['product_name']?.toString() ?? '',
      productType: json['product_type']?.toString() ?? '',
      farmerName: json['farmer_name']?.toString() ?? '',
      ...
      organic:
          json['organic'] == true ||
          json['organic']?.toString().toLowerCase() == 'true',
      metaHash: json['meta_hash']?.toString(),
      createdAt: json['created_at'] != null
          ? DateTime.tryParse(json['created_at'].toString())
          : null,
